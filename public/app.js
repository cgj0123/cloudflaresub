...
(function() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (rocketUrl) rocketUrl.value = data.urls.raw;
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (qrButton) {
      ...
      if (!input || !input.value) {
        warningBox.textContent = '请先生成订阅链接';
        warningBox.classList.remove('hidden');
        return;
      }
      if (!window.QRCode) {
        warningBox.textContent = '二维码组件加载失败';
        warningBox.classList.remove('hidden');
        return;
      }
      qrCanvas.innerHTML = '';
      ...
      new window.QRCode(qrCanvas, { text: input.value, width: 220, height: 220, correctLevel: window.QRCode.CorrectLevel.M });
      return;
    }

    if (event.target.closest('[data-close-modal="true"]')) closeQrDialog();
  });
